...
def load_data():
    ...
    df["Payé"] = df["Payé"].astype(str).str.lower().isin(
        ["true", "1", "vrai", "x", "v"]
    )
...
if st.sidebar.button("💾 Enregistrer"):
    ...
    if type_op != "Remboursement à Julie":
        ...
        new_row = {
            ...
            "Description": desc,
            "Montant": valeur,
            "Payé": False,  # un gain n'est JAMAIS payé à l'entrée
    ...
    # ---------------- REMBOURSEMENT JULIE ----------------
        ...
        new_row = {
            ...
            "Description": desc,
            "Montant": -montant,
            "Payé": True,  # le remboursement est payé
            "Année": str(date_op.year),
        }
        ...
        # 4️⃣ Total réellement remboursé (APRES ajout)
        total_rembourse = abs(
            df[df["Type"] == "Remboursement à Julie"]["Montant"].sum()
        )

        # 5️⃣ CONDITION UNIQUE ET CORRECTE
...
gains_attente = df_all[
    (df_all["Montant"] > 0) & (df_all["Payé"] == False)
]["Montant"].sum()

julie_reste = gains_attente / 2
julie_paye = abs(
    df_all[df_all["Type"] == "Remboursement à Julie"]["Montant"].sum()
)
